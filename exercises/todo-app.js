...
function insertListItem(item) {
 /* update DOM ol */
	...
	li.innerHTML = `<span class="item-name" id="${item.id}">${item.name} </span><span class="actions"><input class="checkbox" type="checkbox" onchange="onCompleted(event)" data-id="${item.id}"/><i class="fas fa-trash fa-lg" data-id="${item.id}" onclick="onItemDelete(event)"></i></span>`;
...
function onItemAdd (e) {
	if (e.key == 'Enter') {
		...
		const newItem = {name: e.target.value, completed: false, id: new Date().getTime()}
